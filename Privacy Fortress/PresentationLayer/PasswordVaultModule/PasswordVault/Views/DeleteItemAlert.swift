...
struct DeleteItemAlert: View {
            ...
            if isPresented {
                ...
                VStack(spacing: 0) {
                    ...
                    Text("Delete password?")
                        ...
                        .padding(.bottom, 4)
                    Text("Login informations will be deleted, are you sure with?")
                        .foregroundColor(ColorManager.textDefaultColor.color)
                    ...
                    VStack(spacing: 0) {
                        Rectangle()
                            ...
                            .frame(height: 1)
                        Button(action: {
                            closeAlert(isNeedToDelete: true)
                        }) {
                            Text("Yes")
                        ...
                        Rectangle()
                            ...
                            .frame(height: 1)
                        Button(action: {
                            closeAlert(isNeedToDelete: false)
                        }) {
                            Text("Cancel")
                                .frame(maxWidth: .infinity, maxHeight: .infinity)
                                .foregroundColor(ColorManager.textBlueColor.color)
    ...
    private func closeAlert(isNeedToDelete: Bool) {
        withAnimation {
            scaleEffect = 0.8
            opacity = 0.0
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
            isPresented = false
            onDismiss?(isNeedToDelete)
        }
    }
}

#Preview {
    DeleteItemAlert(isPresented: .constant(true))
}
