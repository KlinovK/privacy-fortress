...
struct RestoreMediaAlert: View {
            ...
            if isPresented {
                ...
                VStack(spacing: 0) {
                    ...
                    Text("Restore media?")
                        ...
                        .padding(.bottom, 4)
                    Text("Do you want to recover media on your phone and delete it from Media Safe?")
                        .foregroundColor(ColorManager.textDefaultColor.color)
                    ...
                    VStack(spacing: 0) {
                        Rectangle()
                            ...
                            .frame(height: 1)
                        Button(action: {
                            closeAlert(isNeedToRestore: true)
                        }) {
                            Text("Yes")
                        ...
                        Rectangle()
                            ...
                            .frame(height: 1)
                        Button(action: {
                            closeAlert(isNeedToRestore: false)
                        }) {
                            Text("No, delete forever")
                                .frame(maxWidth: .infinity, maxHeight: .infinity)
                                .foregroundColor(ColorManager.textBlueColor.color)
                                .cornerRadius(8)
    ...
    private func closeAlert(isNeedToRestore: Bool) {
        withAnimation {
            scaleEffect = 0.8
            opacity = 0.0
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
            isPresented = false
            onDismiss?(isNeedToRestore)
        }
    }
}

#Preview {
    RestoreMediaAlert(isPresented: .constant(true))
}
